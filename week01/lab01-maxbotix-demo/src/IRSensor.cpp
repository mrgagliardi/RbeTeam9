...
void IRSensor::init(uint8_t interfaces)
{
    if(interfaces & USE_ECHO)
    {
        // assert ECHO pin is an input
        pinMode(PULSE_PIN, INPUT);
        attachInterrupt(PULSE_PIN, ISR_IRSensor, CHANGE);
    }
 ...
 */
uint8_t IRSensor::checkPingTimer(void)
{
...
uint16_t IRSensor::checkEcho(void)
{
...
uint16_t IRSensor::readMCP3002(bool force)
{
    uint16_t retVal = 0;
...
uint16_t IRSensor::readASCII(void)
{
 ...
 */
bool IRSensor::getDistance(float& distance)
{
    distance = -99;
    return false;
}